...
#define RTISRCOUNT_LAY(RTCSTR, NEXTTASKNUM)                                     \
...
#define RTREGIO_LAY(RTRIOSTR,NEXTTASKNUM,HWREGADD,BYTES,DATA_P)             \
...
#define RTFLAGIO_LAY(RTFIOSTR,NEXTTASKNUM,HWREGADD,MASK,SHIFT,DATA_P)       \
...
typedef struct {
    uint8_t SPDATAR;  // SPI Data Register
    uint8_t Total;    // Total Bytes of Registers
} SPIRegData_t;
...
#define SPI_HW_LAY()                                                          \
    ...
    {                                                                         \
        HWFgGpPara_t SPIFGParaList[7] = {                                     \
            {&SPSR, 0x01, 0}, {&SPCR, 0x03, 0}, {&SPCR, 0x04, 2},             \
            {&SPCR, 0x08, 3}, {&SPCR, 0x10, 4}, {&SPCR, 0x20, 5},             \
            {&DDRB, 0x0F, 0}}; /*define HWFlagGroup Parameters and initial it \
                                */                                            \
        HWRegPara_t SPIRegParaList[1] = {                                     \
            {&SPDR, 1}}; /*define HWREG Parameters and initial it */          \
        SPIFgGpData_t SPIFgGpData_str = SPI_FG_DATA_INT;                      \
        SPIRegData_t SPIRegData_str = SPI_REG_DATA_INT;                       \
    ...
    SPIHWSet_str.FgGpNum = 7; /*HW Setting Flag-group number*/                \
    SPIHWSet_str.RegNum = 1;  /*HW Setting Register Number*/
...
#define TWI_HW_LAY()                                              \
    ...
    {                                                             \
        static HWFgGpPara_t TWIFGParaList[6] = {                  \
            {&TWSR, 0x03, 0}, {&TWCR, 0x01, 0}, {&TWCR, 0x04, 2}, \
            {&TWAR, 0xFE, 1}, {&TWAR, 0x01, 0}, {&DDRD, 0x03, 0}, \
        };                                                        \
...
#define TIM2_HW_LAY()                                                          \
    ...
    HardWareSet_t TIM2HWSet_str = {                                            \
        .FgGpNum    = 4,                 /*HW Setting Flag-group number*/      \
        .RegNum     = 1,                 /*HW Setting Register Number*/        \
 ...
 */
#define PWM0_HW_LAY()                                                          \
    HWFgGpPara_t PWM0FGParaList[5]  = {{&ASSR, 0x08, 3},                       \
                                      {&TCCR0, 0x48, 3},                      \
                                      {&TCCR0, 0x07, 0},                      \
                                      {&TCCR0, 0x30, 4},                      \
                                      {&DDRB, 0x10, 4}};                      \
    ...
    HardWareSet_t PWM0HWSet_str     = {.FgGpNum    = 5,                        \
                                   .RegNum     = 1,                        \
 ...
 */
#define PWM2_HW_LAY()                                                          \
    HWFgGpPara_t PWM2FGParaList[4]  = {{&TCCR2, 0x48, 3},                      \
                                      {&TCCR2, 0x07, 0},                      \
                                      {&TCCR2, 0x30, 4},                      \
                                      {&DDRB, 0x80, 7}};                      \
    ...
    HardWareSet_t PWM2HWSet_str     = {.FgGpNum    = 4,                        \
                                   .RegNum     = 1,                        \
 ...
 */
#define PWM1_HW_LAY()                                                          \
    HWFgGpPara_t PWM1FGParaList[9] = {                                         \
        {&TCCR1A, 0x03, 0}, {&TCCR1B, 0x18, 3}, {&TCCR1B, 0x07, 0},            \
        {&TCCR1A, 0xc0, 6}, {&TCCR1A, 0x30, 4}, {&TCCR1A, 0x0c, 2},            \
        {&DDRB, 0xe0, 5},   {&DDRB, 0x40, 6},   {&DDRB, 0x80, 7}};             \
    ...
    HardWareSet_t PWM1HWSet_str     = {.FgGpNum    = 9,                        \
                                   .RegNum     = 4,                        \
...
#define PWM3_HW_LAY()                                                          \
    ...
    {                                                                          \
        static HWFgGpPara_t PWM3FGParaList[9] = {                              \
...
/*-- ExecutionServiceProvider section start ----------------------------------*/

/**
 * @brief   Pipeline 結構體
 * @ingroup pipeline_struct
 */
typedef struct {
    uint8_t MaxTask;                    /* Maximum capacity for tasks to be Registed    */
    uint8_t Total;                      /* Total number of Registed Tasks               */
    volatile uint8_t TaskId;            /* Task Identification gotten                   */
	volatile HWIntStr_t* HWIStr_ssp;    /* pointer of HW interrupt structure            */
    volatile TaskBlock_t* Task_p;       /* pointer of Task List                         */
    volatile FifoBFStr_t* FifoBF_p;     /* FIFO Buffer for waiting list of SETask       */
    volatile ScopeBFStr_t* ScopeBF_p;   /* Circular buffer to record executed TaskId    */
} PipelineStr_t;

/**
 * @brief 供使用者呼叫將功能方塊登錄成可由管道工作執行器代執行的工作方塊。執行工作包括: 
 *          1. 取得新工作編號。
 ...
 * @return              工作識別碼。
 */
uint8_t Pipeline_reg(PipelineStr_t* PLStr_ssp, Func_t FbFunc_p, void* FbPara_p, uint8_t* TaskName_p);

/**
 * @brief 能夠由先進先出緩衝器取得工作識別碼，據以由工作方塊表中取得工作方塊並予以執行。
 ...
 * 
 * @param TASKNUM       Maximum Number of Tasks allow to be registed into Pipeline executor.
...
FIFOBUFF_LAY(PipelineFifo_str,FBDEPTH);              /* LayOut FIFO BUFFER */                    \
SCOPEBUFF_LAY(PipelineScope_str,SBDEPTH,SBDEPTH);    /* LayOut SCOPE BUFFER PRECOUNT=DEPTH */    \
PipelineStr_t Pipeline_str = {0};                               /* define structure */                      \
{                                                                                                           \
    static TaskBlock_t Pipeline_TASKLIST[TASKNUM];              /* define task list */                      \
    Pipeline_str.MaxTask   = TASKNUM;                           /* Initial the Task str list length */      \
    Pipeline_str.Task_p    = Pipeline_TASKLIST;                 /* Net the Task str list */                 \
    Pipeline_str.FifoBF_p  = &PipelineFifo_str;                 /* Net the FIFO BUFFER */                   \
    Pipeline_str.ScopeBF_p = &PipelineScope_str;                /* Net the Scope BUFFER */                  \
}

/**
 * @brief   提供前級工作方塊，於完成本身工作之後，叫用以便將其後級的後續工作指標，放進先進出緩衝區等待管道工作執行器執行。
 * @ingroup trig_next_task_macro
 * 
 * @param TASKSTRP      The Task Structure pointer of running task.
 * @param NEXTTASKINDEX Index of next Tasks.
 */
#define TRIG_NEXT_TASK(TASKSTRP,NEXTTASKINDEX)                                                                          \
{                                                                                                                       \
    uint8_t NextTaskId = NEXTTASKINDEX;                                       /* Get Next TaskId */                   \
    Pipeline_str.Task_p[NextTaskId].state++;                                  /* trigger state up count */            \
    FifoBF_put((FifoBFStr_t*)(Pipeline_str.FifoBF_p), (void*)&NextTaskId, 1); /* PUT Next TaskID into FIFO BUFFER */     \
}

/*-- ExecutionServiceProvider section end ------------------------------------*/
